...
@testset "MeasurementSettings Tests" begin
    ...
    @testset "Test 1: Creating Measurement Settings" begin
        @test measurement_settings.N == N
        @test measurement_settings.NU == NU
        @test length(measurement_settings.site_indices) == N

        # Validate dimensions of local unitaries
        @test size(measurement_settings.local_unitaries, 1) == NU
        @test size(measurement_settings.local_unitaries, 2) == N
    end
    ...
    @testset "Test 5: Different Ensembles" begin
        ...
        for ensemble in ensembles
            settings = LocalUnitaryMeasurementSettings(N, NU, ensemble=ensemble)
            @test settings.N == N
            @test settings.NU == NU
            @test length(settings.site_indices) == N
        end
    end
